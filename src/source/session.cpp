  ...
  while (running) {
    ...
    for (int i = 0; i < cnt; i++) {
      if (triggeredEvent[i].data.fd == 0) {
        ...
        state.sendAck = false;
        dissect(readCount);
        if (state.sendAck) encapsulate(""); // if we receive the secret message from server,then we need to send ack
        if (!secret.empty() && state.recvPacket) { // send a normal message to sever
          encapsulate(secret);
          secret.clear();
        }
      }
...
int Session::encapsulateTCP(std::span<uint8_t> buffer, const std::string& payload) {
  ...
  hdr.ack_seq = htonl(state.tcpseq);// original tepseq + PayloadSize
